...
import type { CapacityMeeting, MyCapacityResponse, ProjectGroup } from '../types';
import { WEEKLY_CAPACITY, projectColor, statusBadgeColor, MEETING_COLOR } from '../types';
      ...
      >
        ...
        {view === 'summary' && data && (
          <div className="flex flex-col flex-1 min-h-0 gap-6 mt-4">
            ...
            {/* Breakdown — projects THEN meetings, inside ONE scroll region so
                both sections share the available height (hero tiles + footer
                stay pinned via shrink-0). The project list must NOT own an
                internal flex-1 scroll of its own: as a flex sibling of the
                meetings block it collapsed to ~0h whenever meetings existed,
                hiding the project cards. */}
